...
**The welcome step reuses the existing durable field.** Its exact DeepSeeker copy and version live in `onboarding-copy.ts`. Loopback clients compare and write `ui-onboarding.welcomeNoticeVersion` through the existing settings API, and only Get started acknowledges the current version. Remote clients retain the existing process-local fallback because the settings namespace is loopback-only. No Host schema, API-proxy allowlist, or persistence implementation changes.
...
A fresh loopback profile sees the DeepSeeker welcome, then an inline DeepSeek key dialog only when no provider is usable. Acknowledgement remains versioned in `settings.yaml`, secrets remain write-only in `.credentials.yaml`, and already-ready or unsupported deployments render no onboarding chrome while readiness loads. The Models package owns product-onboarding presentation as well as provider configuration; its README and browser coverage make that responsibility explicit. This decision keeps concise first-run context after the historical [full-viewport beta notice removal](../simplification/2026-08-13-remove-first-run-beta-notice.md) without restoring telemetry copy or takeover layout.
